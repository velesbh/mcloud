 ...
 * Java 8  — ≤ 1.16
 */
export function requiredJavaMajor(gameVersion) {
    const [maj, min] = gameVersion.split(".").map(Number);
    if (maj > 1 || (maj === 1 && min >= 21))
        return 21;
    if (maj === 1 && min >= 17)
        return 17;
    return 8;
 ...
 */
export function javaBinForMajor(major) {
    const candidates = [
    ...
    try {
        const found = execSync(`find /usr/lib/jvm -name "java" -path "*java-${major}*" -type f 2>/dev/null | head -1`, { encoding: "utf8", timeout: 5000 }).trim();
        if (found)
            return found;
    }
    catch { /* ignore */ }
 ...
 */
function runStreamed(cmd, args, env, onLine) {
 ...
 */
export async function installJava(gameVersion, serverId, overrideMajor) {
    ...
    const updateCode = await runStreamed("apt-get", ["update", "-qq"], env, (l) => void emit(l));
    if (updateCode !== 0) {
    ...
    const installCode = await runStreamed("apt-get", ["install", "-y", pkg], env, (l) => void emit(l));
    if (installCode !== 0) {
...
//# sourceMappingURL=java-installer.js.map