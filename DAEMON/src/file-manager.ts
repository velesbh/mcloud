...
/**
 * File manager works over Supabase Realtime as a request/response channel:
 *
 *   Client                                    Daemon
 *     |--- broadcast fm:{server_id} {req} --->|
 *     |   {reqId, op, args}                   |
 *     |                                       |
 *     |<-- broadcast fm:{server_id} {res} ----|
 *     |   {reqId, ok, data?, error?}          |
 *
 * Each request must include a `reqId` so the client can match the response.
 */
...
async function handleList(serverId: string, rel: string): Promise<FileEntry[]> {
...
async function handleRead(serverId: string, rel: string): Promise<{ content: string; size: number }> {
...
async function handleWrite(serverId: string, rel: string, content: string) {
  const abs = safePath(serverId, rel);
  await mkdir(path.dirname(abs), { recursive: true });
  await writeFile(abs, content, "utf8");
}

async function handleMkdir(serverId: string, rel: string) {
  const abs = safePath(serverId, rel);
  await mkdir(abs, { recursive: true });
}

async function handleDelete(serverId: string, rel: string) {
  const abs = safePath(serverId, rel);
  await unlink(abs);
}

async function handleRename(serverId: string, from: string, to: string) {
...
async function dispatch(serverId: string, req: Op) {
  switch (req.op) {
    case "list":   return await handleList(serverId, req.path);
    case "read":   return await handleRead(serverId, req.path);
    case "write":  await handleWrite(serverId, req.path, req.content); return { ok: true };
    case "mkdir":  await handleMkdir(serverId, req.path); return { ok: true };
    case "delete": await handleDelete(serverId, req.path); return { ok: true };
    case "rename": await handleRename(serverId, req.from, req.to); return { ok: true };
  }
}
...
export function subscribeFileManager(serverId: string) {
  if (subscribed.has(serverId)) return;
  ...
  channel.on("broadcast", { event: "req" }, async (msg) => {
    const payload = msg.payload as Req | undefined;
    if (!payload?.reqId || !payload.request) return;
    try {
