...
import { readFile, mkdir, writeFile, unlink, cp } from "node:fs/promises";
...
interface ModpackFile {
  path: string;          // relative install path inside server dir
  hashes?: { sha1?: string; sha512?: string };
  downloads: string[];
  fileSize?: number;
  env?: { client?: string; server?: string }; // "required" | "optional" | "unsupported"
}
...
async function downloadTo(url: string, dest: string): Promise<void> {
  ...
  if (!res.ok || !res.body) throw new Error(`fetch ${res.status} ${url}`);
 ...
 */
export async function installModpack(serverId: string, modpackUrl: string): Promise<void> {
  ...
  if (!indexEntry) throw new Error("modpack missing modrinth.index.json");

  const index: ModpackIndex = JSON.parse(indexEntry.getData().toString("utf8"));
  ...
  const serverFiles = index.files.filter(
    (f) => !f.env?.server || f.env.server !== "unsupported"
  );
    ...
    try {
      ...
      if (done % 5 === 0 || done === serverFiles.length) {
        await broadcastConsole(
          serverId,
          `[modpack] Downloaded ${done}/${serverFiles.length} mods`,
          "system"
        );
      }
    } catch (err) {
